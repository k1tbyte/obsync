...
const nodeProbe: LinkProbe = (absolute) => {
	try {
...
beforeAll(() => {
	vault = mkdtempSync(join(tmpdir(), "obsync-symlink-"));
	mkdirSync(join(vault, "real"));
	writeFileSync(join(vault, "real", "note.md"), "hi");
	symlinkSync(join(vault, "real"), join(vault, "linked"), "dir");
	symlinkSync(join(vault, "real", "note.md"), join(vault, "alias.md"), "file");
});

describe("symlinkDetector", () => {
	it("leaves ordinary files and folders alone", () => {
		const detector = symlinkDetector(vault, nodeProbe);
	...
	it("flags a linked file", () => {
		expect(symlinkDetector(vault, nodeProbe).isLink("alias.md")).toBe(true);
	});

	it("treats a missing path as ordinary", () => {
		expect(symlinkDetector(vault, nodeProbe).isLink("gone/x.md")).toBe(false);
	});
