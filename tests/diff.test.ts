...
import { diff, type DiffInput } from "../src/sync/diff";
import { EChangeType, EFileKind, type Manifest, type LocalSnapshot, type ManifestEntry } from "../src/types";

function mockManifest(snapshotId: string, files: Record<string, Partial<ManifestEntry>> = {}): Manifest {
    const filledFiles: Record<string, ManifestEntry> = {};
    for (const [k, v] of Object.entries(files)) {
        filledFiles[k] = { hash: null as any, size: 0, mtime: 0, kind: EFileKind.Vault, ...v };
    }
    return {
        version: 1,
        vaultId: "vault",
...
function mockLocal(files: Record<string, Partial<ManifestEntry>> = {}): LocalSnapshot {
    const filledFiles: Record<string, ManifestEntry> = {};
    for (const [k, v] of Object.entries(files)) {
        filledFiles[k] = { hash: null as any, size: 0, mtime: 0, kind: EFileKind.Vault, ...v };
    }
